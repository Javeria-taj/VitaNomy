...
export function PatientBanner() {
  ...
  const riskColor = analysis?.overall_risk === 'CRITICAL' ? 'bg-red-50 text-red-600 border-red-200' : 
                    analysis?.overall_risk === 'HIGH' ? 'bg-orange-50 text-orange-600 border-orange-200' :
                    analysis?.overall_risk === 'MEDIUM' ? 'bg-amber-50 text-amber-600 border-amber-200' :
    ...
    <div className="flex items-center gap-3.5 p-4 rounded-xl bg-bg-secondary border-[0.5px] border-border-tertiary mb-5 shadow-sm">
      ...
      <div>
        ...
        <div className="text-[12px] text-text-secondary mt-0.5 font-medium">
          {patient.age} yr · {patient.gender.charAt(0).toUpperCase() + patient.gender.slice(1)} · BMI {bmi} · {patient.smoking ? 'Smoker' : 'Non-smoker'} · {patient.exercise.charAt(0).toUpperCase() + patient.exercise.slice(1)} exercise
        </div>
      </div>
      {analysis && (
